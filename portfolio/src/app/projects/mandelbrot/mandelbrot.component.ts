...
export class MandelbrotComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
                  ...
                  for (let row = 0; row < data.height; row++) {
                    ...
                    if (data.isMobile) {
                      ...
                      x =
                        data.img_set_start +
                        (col / data.width) *
                          (data.img_set_end - data.img_set_start);
                      y =
                        data.real_set_start +
                        (row / data.height) *
                          (data.real_set_end - data.real_set_start);
                    } else {
                      x =
                        data.real_set_start +
                        (col / data.width) *
                    ...
                    let mandelbrot_calculation = mandelbrot(
                      c,
                      isMobile,
                      max_iterations
                    );
    ...
    const createPane = () => {
      ...
      folder.addBinding(params, "max_iterations", {
        min: 100,
        max: 2000,
        step: 1,
      });
      ...
      info.on("click", () => {
        ...
        if (this.infoSidenav.opened) {
          document
            .getElementsByClassName("cdk-drag-handle")[0]
      ...
      set_center.on("click", () => {
        ...
        let zoom_factor_real =
          (params.real_set_end - params.real_set_start) / params.zoom_factor;
        let zoom_factor_img =
          (params.img_set_end - params.img_set_start) / params.zoom_factor;
        ...
        params.a = (
          (params.real_set_end + params.real_set_start) /
          2.0
        ).toPrecision(17);
        params.b = (
          (params.img_set_end + params.img_set_start) /
          2.0
        ).toPrecision(17);
        this.pane.refresh();
      ...
      coords.on("click", () => {
        ...
        downloadPositionLink.setAttribute(
          "href",
    ...
    this.canvas.addEventListener("click", (e) => {
      ...
      const canvasRelativeX =
        (elementRelativeX * this.canvas.width) / rect.width;
      const canvasRelativeY =
        (elementRelativeY * this.canvas.height) / rect.height;
      // zoom amount in complex magnitude, standard real as x and img as y
      let zoom_factor_real =
        (params.real_set_end - params.real_set_start) / params.zoom_factor;
      let zoom_factor_img =
